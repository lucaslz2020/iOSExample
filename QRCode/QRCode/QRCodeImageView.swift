...
class QRCodeImageView: UIView {

    lazy var QRCodeView: UIImageView = {
        let result: UIImageView = UIImageView(frame: self.bounds)        
        
        let QRImage = QRCodeImageView.createQRCodeToSolveBlur("http://lucaslz.com", withSize: self.bounds.size)
        
        let logoImage = UIImage(named: "avatar")
        result.image = QRCodeImageView.addLogoImage(logoImage!, toImage: QRImage!, logoSize: CGSizeMake(50, 50))
